...
\hspace{-0.7mm}\le\hspace{-0.7mm} \frac{2\gamma}{1\hspace{-0.6mm}-\hspace{-0.6mm}\gamma^2}\Big( \gamma^k
%	 ||\hat V_1 \hspace{-0.6mm}-\hspace{-0.6mm} \hat V_0||_{\infty} 
	 \alpha
	 \hspace{-0.6mm}+ \hspace{-0.6mm}2|| V^* \hspace{-0.6mm}-\hspace{-0.6mm} \tilde V^* ||_{\infty} 
	 \hspace{-0.6mm}+\hspace{-0.6mm}\epsilon ||\tilde V^*||_{\cF}\hspace{-0.6mm} \Big). %\label{ICMLbound}
\end{align*}
\end{theorem}
...
If we approximate the CME (\ref{muDef}) with $\hat \mu^{KS}(s,a) = \sum_{i=1}^n \alpha^{KS}_i(s,a) \phi(s'_i)$ using kernel smoothing, the induced finite transition dynamics are
\begin{align}
\alpha^{KS}_i(s,a) :=  \frac{K((s,a),(s_i,a_i))}{\sum_{j=1}^n K((s,a),(s_j,a_j))} \label{KSalphas}
\end{align}
where $K$ is a smoothing kernel, and condition (\ref{L1Constraint}) holds. The KBRL method uses a similar model, though there no smoothing is performed over actions (in fact the method allows for a different kernel per action). The KBRL method is shown to be consistent: under smoothness conditions, the learned value function will converge to the optimal value function with infinite i.i.d. data.

One drawback is that the size of the state space of the induced MDP equals $|\cD|=n$ and planning via a solving the approximate Bellman equation (\ref{approxBellman}) scales poorly with the data. Versions of KBRL with efficient planning have been proposed using stochastic factorization of the transition matrix of the induced finite MDP \citep{\BarretoStochasticFactorization} or a cover tree quantization of the state space \citep{\KvetonRepresentativeKBRL}.
...
To ensure a contraction we obtain the proper CME $\hat\mu^{PCMP}(\cdot)$ by projecting the learned weights to the L1-ball: denoting ${\rm proj}(f)\hspace{-0.5mm}:=\hspace{-0.5mm}\argmin_{\bbeta \in \R^m, ||\bbeta||_1\le 1}\hspace{-0.3mm} || f \hspace{-0.3mm}-\hspace{-0.3mm} \sum_{j=1}^m \beta_j  \phi(c_j)  ||_{\cF}$ we set
\begin{align}
\balpha^\bot(s,a) := {\rm proj}(\hat \mu^{CMP}(s,a) ),\label{CompProjAlpha}
\end{align}
which can be done using a Lasso (see Appendix~\ref{LassoSec}), and finally we normalize,\todoc{This is confusing. Is it because of numerical issues? Anyhow, this must be explained.}\todog{It makes the pseudo-MDP closer to an MDP. I think what happens otherwise maybe that some regions $||\balpha(s,a)||_1$ is very small, and everything degenerates, and its practically better to just to enforce an actual MDP. I don't think its a numerical thing -- I don't get singularity errors without it.}
\begin{align}
\balpha^{PCMP}(s,a):=\balpha^\bot(s,a)/||\balpha^\bot(s,a)||_1.\label{NormAlpha}
\end{align}
...
\begin{subfigure}[b]{\figwidth}
%\centering
\includegraphics[width=\textwidth]{MCrewardsb.png}
%\caption{aaaaa}
%\label{figCPrwd}
\end{subfigure}
\begin{subfigure}[b]{\figwidth}
%\centering
\includegraphics[width=\textwidth]{MCplanningb.png}
%\caption{aaaa}
%\label{figCPp}
\end{subfigure}
\begin{subfigure}[b]{\figwidth}
%\centering
\includegraphics[width=\textwidth]{MCmodelfeatureb.png}
%\caption{aaaa}
%\label{figCPmf}
\end{subfigure}
...
\begin{subfigure}[b]{\figwidth}
%\centering
\includegraphics[width=\textwidth]{Quadrewardsb.png}
%\caption{aaaa}
%\label{figCPp}
\end{subfigure}
\begin{subfigure}[b]{\figwidth}
%\centering
\includegraphics[width=\textwidth]{Quadplanningb.png}
%\caption{aaaa}
%\label{figCPp}
\end{subfigure}
\begin{subfigure}[b]{\figwidth}
%\centering
\includegraphics[width=\textwidth]{Quadmodelfeatureb.png}
%\caption{aaaa}
%\label{figCPmf}
\end{subfigure}
...
The state kernel is a Gaussian $L(s,s') = \exp\left({\frac{-1}{2\sigma_{\cS}^2}(s - s')^\top \bM_\cS (s - s')}\right)$ with $\bM_\cS = diag(1,1/4)$, and the state-action kernel is  $K((s,a),(s',a')) = \exp\left({\frac{-1}{2\sigma_{\cS\times\cA}^2}((s,a) - (s',a'))^\top \bM_{\cS\times\cA} ((s,a) - (s',a'))}\right)$ with $\bM_{\cS\times\cA} = diag(1,1/4,1/10000)$. The output feature map $\phi(s) = L(s,\cdot)$ with $\sigma_{\cS} = 0.5$ (chosen by informal search). During model learning we performed 5-fold cross validation over a range of 10 bandwidths in the range $[0.01,5]$ to optimize $\sigma_{\cS\times\cA}$. For the compressed CME the tolerance of the compression set was set to $\delta = 0.1$, i.e. we use a $\delta$-lossy compression set $\cC$ with $\delta=0.1$.
...
The state kernel is a Gaussian $L(s,s') = \exp\left({\frac{-1}{2\sigma_{\cS}^2}(s - s')^\top \bM_\cS (s - s')}\right)$ with $\bM_\cS = diag(1,100)$, and the state-action kernel is  $K((s,a),(s',a')) = \exp\left({\frac{-1}{2\sigma_{\cS\times\cA}^2}((s,a) - (s',a'))^\top \bM_{\cS\times\cA} ((s,a) - (s',a'))}\right)$ with $\bM_{\cS\times\cA} = diag(1,100,1/25)$. The output feature map $\phi(s) = L(s,\cdot)$ with $\sigma_{\cS} = 0.5$ (chosen by informal search). During model learning we performed 5-fold cross validation over a range of 10 bandwidths in the range $[0.01,5]$ to optimize $\sigma_{\cS\times\cA}$. For the compressed CME the tolerance of the compression set selection was set to $\delta = 0.01$, i.e. we use a $\delta$-lossy compression set $\cC$ with $\delta=0.01$.

\subsection{Quadrocopter Experiment Details}

The state kernel is a Gaussian $L(s,s') = \exp\left({\frac{-1}{2\sigma_{\cS}^2}(s - s')^\top \bM_\cS (s - s')}\right)$ with $\bM_\cS =diag( \bone/5, \bone/10^5 ,\bone, \bone/10^5 ,0)$ where $\bone = (1,1,1)$, (which essentially reduces the state observations to 6 dimensions), and the state-action kernel is
\begin{align}
&K((s,a),(s',a')) \nn\\
...
\end{align}
with $\bM_{\cS\times\cA} =diag(\bone /5,\bone/10^5 ,\bone,\bone/10^5,0,  \bone/100)$. The output feature map $\phi(s) = L(s,\cdot)$ with $\sigma_{\cS} = 1$ (chosen by informal search). During model learning we performed 5-fold cross validation over a range of 10 bandwidths in the range $[0.01,5]$ to optimize $\sigma_{\cS\times\cA}$. For the compressed CME the tolerance of the compression set selection was set to $\delta = 0.1$, i.e. we use a $\delta$-lossy compression set $\cC$ with $\delta=0.1$.
