...
\begin{algorithmic}
   ...
   \STATE {\bf Parameters:} Kernel $K$ on $\cS\times\cA$; feature map $\phi(\cdot)$ on $\cS$; compression tolerance $\delta$; policy improvements $J$; $n_{\rm new}$; maximum basis dimension $d$.\\
	...
	\FOR {$k=1,2,...$}	 
	 ...
	 \STATE {\bf Define CME $\mu^{PCMP}$:} $\balpha^{CMP}(s,a) = \psi^k(s,a)^\top \bW^{CMP}_k$, project $\balpha^{PCMP}(s,a) = {\rm proj}(\hat \mu^{CMP}(s,a) )$ (\ref{CompProjAlpha}) and normalize $\balpha^{PCMP}(s,a) \leftarrow \balpha^{PCMP}(s,a)/||\balpha^{PCMP}(s,a)||_1$.
	...
	\FOR {$j=1,2,...,J$}
   \STATE {\bf Policy evaluation:} Using finite pseudo-MDP dynamics $\balpha^{PCMP}(s_i',a)$ for all $a\in\cA$, $s_i'\in\cD_k$ solve approximate Bellman Equation (\ref{approxBellman}) to obtain estimate $\hat V_j$ of $V^{\pi_k}$ at the sample points $s_i'\in\cD_k$. Set $\hat Q_j(s,a) = r(s,a) + \gamma \sum_{j=1}^{n_k} \alpha^{PCMP}_j(s,a) \hat V_j(s_i')$. \todoc[inline]{$\alpha_j$ is undefined.}\todog{Fixed.}
