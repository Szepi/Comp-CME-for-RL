...
\end{align}
\todoc[inline]{I wish we could add a sentence to explain normalization, something like:
Normalization, while not necessary for stability, is expected to help as in the long run $\balpha^{PCMP}$ is expected to have a norm close to one.}
...
\end{align}
where $\balpha(s,a) = \psi(s,a)^\top \bW$, and the $d\times n$ matrix $\bW = \left( \bw_1, \bw_2, ... , \bw_n \right)$ must be optimized. The following equivalent form will be useful (the derivation is in the Appendix):
...
We first suppose that a good compression set $\cC$ is known and consider solving a CME of the form $\mu(s,a) = \sum_{j=1}^m \alpha_j(s,a) \phi(c_j)$, where $\balpha(s,a) = \psi(s,a)^\top \bW$. We are interested in the following optimization problem for the model
\begin{align}
