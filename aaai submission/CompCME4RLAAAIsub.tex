...
%Further, these methods have so far been studied in the batch setting in which good data is provided a-priori the learning process. These are drawbacks in reinforcement learning where data typically arrives online during an agents interaction with the system.
...
\begin{algorithmic}
   \STATE {\bf Input:}  Initial compression set $\cC = {c_1,...,c_m}$, candidates $\cP = {s'_1,...,s'_n}$, tolerance $\delta$
	 ...
	 \STATE {\bf Augment compression set:} $\cC \leftarrow \cC \cup s'_j$
   \ENDIF
	\ENDFOR
\end{algorithmic}
\end{algorithm}
...
\begin{algorithmic}
   ...
   \STATE {\bf Parameters:} Kernel $K$ on $\cS\times\cA$; feature map $\phi(\cdot)$ on $\cS$; compression tolerance $\delta$; policy improvements $J$; $n_{\rm new}$; maximum basis dimension $d$.\\
	...
	\FOR {$k=1,2,...$}	 
	 ...
	 \STATE {\bf Augment compression set:} Look for new compression points in the new data, $\cC_k = \mbox{augmentCompressionSet}(\cC_{k-1},\{s'_i\}_{i=n_{k-1}+1}^{n_k},\delta)$.
	 \STATE {\bf Augment candidate feature dictionary:} Add kernel functions on new data to set of candidate features $\cG_k = \cB_k \cup \{ K(\cdot,(s_i,a_i)) \}_{i=n_{k-1}+1}^{n_k}$
	 ...
	 \STATE {\bf Project weights to enforce contraction mapping:} $\alpha^{PCOMP}(s_i',a) = {\rm proj}(\alpha(s_i',a))$ by solving (\ref{CompProjAlpha}), finally normalize $\balpha^{PCOMP}(s,a) \leftarrow \balpha^{PCOMP}(s,a)/||\balpha^{PCOMP}(s,a)||_1$.
	\FOR {$j=1,2,...,J$}
   \STATE {\bf Policy evaluation:} Form estimate $\hat V_j$ of $V^{\pi_k}$ by solving the approximate Bellman Equation (\ref{approxBellman}) at the sample points $s_i'\in\cD_k$. Set $\hat Q_j(s,a) = r(s,a) + \gamma \sum_{j=1}^{n_k} \alpha_j(s,a) \hat V_j(s_i')$.
